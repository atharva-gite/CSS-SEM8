...
class HMACCalculator:
    ...
    def next_step(self):
        if self.current_step == 0:
            ...
            self.instruction_label.config(
                text=f"Step {self.current_step}: Enter this in 'Your text' field to get q: {p}")
            self.input_entry.delete(0, tk.END)
            self.input_entry.insert(0, p)
            self.current_step += 1
        elif self.current_step == len(self.chunks) + 3:
            q = self.result_entry.get()
            z_final = self.z_values[-1]
            r = q + z_final
            self.instruction_label.config(
                text=f"Final Step: Enter this in 'Your text' field to get final HMAC tag t: {r}")
            self.input_entry.delete(0, tk.END)
