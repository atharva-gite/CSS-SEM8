...
# Constants: first 64 bits of the fractional parts of the cube roots of the first 80 primes
K = [
...
def parse_blocks(padded_message):
    return [padded_message[i:i + 128] for i in range(0, len(padded_message), 128)]

# SHA512 main function
def sha512(message):
    H = [
    ...
    blocks = parse_blocks(padded_message)
    ...
    for block in blocks:
        ...
        for t in range(16, 80):
            W[t] = (small_sigma1(W[t - 2]) + W[t - 7] + small_sigma0(W[t - 15]) + W[t - 16]) & ((1 << 64) - 1)
        ...
        H = [(x + y) & ((1 << 64) - 1) for x, y in zip(H, [a, b, c, d, e, f, g, h])]
